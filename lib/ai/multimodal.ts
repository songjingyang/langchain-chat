...
export function createMultimodalMessage(
  ...
  for (const attachment of attachments) {
    if (attachment.type === 'image') {
      enhancedText += `\n\n[图片文件：${attachment.name}，大小：${Math.round(attachment.size / 1024)}KB`;
      
      if (attachment.content?.metadata) {
        const metadata = attachment.content.metadata as { width?: number; height?: number };
      ...
      enhancedText += `\n\n[文件：${attachment.name}，类型：${attachment.type}，大小：${Math.round(attachment.size / 1024)}KB，链接：${attachment.url}]`;
    }
  }
...
export function createTextDescriptionForAttachments(
  ...
  for (const attachment of attachments) {
    if (attachment.type === 'image') {
      description += `\n\n[用户上传了一张图片：${attachment.name}，大小：${Math.round(attachment.size / 1024)}KB`;
      
      if (attachment.content?.metadata) {
        const { width, height } = attachment.content.metadata as { width?: number; height?: number };
        if (width && height) {
      ...
      description += `\n\n[用户上传了文件：${attachment.name}，类型：${attachment.type}，大小：${Math.round(attachment.size / 1024)}KB，链接：${attachment.url}]`;
    }
  }
 ...
 * 获取附件摘要信息
 */
export function getAttachmentsSummary(attachments: MessageAttachment[]): string {
